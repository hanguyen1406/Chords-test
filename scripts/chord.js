const ACTIVE_CHORD_FEATURE = () => {
    ...
    for (let i = 0; i < COMMON_NOTES.e.length; i++) {
        $(".mask.low-e ul").append(
            `<li note-number="${i}">${COMMON_NOTES.e[i]}</li>`
        );
        $(".mask.a ul").append(
            `<li note-number="${i}">${COMMON_NOTES.a[i]}</li>`
        );
        $(".mask.d ul").append(
            `<li note-number="${i}">${COMMON_NOTES.d[i]}</li>`
        );
        $(".mask.g ul").append(
            `<li note-number="${i}">${COMMON_NOTES.g[i]}</li>`
        );
        $(".mask.b ul").append(
            `<li note-number="${i}">${COMMON_NOTES.b[i]}</li>`
        );
    ...
    COMMON_CHORDS.majors.forEach((chord) => {
        $("ul.chord-major").append(
            `<li button-click id="btn-chord-${chord.name}">${chord.name}</li>`
        );
    });
    COMMON_CHORDS.minors.forEach((chord) => {
        $("ul.chord-minor").append(
            `<li button-click id="btn-chord-${chord.name}">${chord.name}</li>`
        );
    });
    ...
    const showChordNumber = (chordSymbol) => {
        console.log("ShowChordNumber: " + chordSymbol);
        if (!canClick) return;
        chordActive =
            COMMON_CHORDS.majors.find((item) => item.name === chordSymbol) ||
            COMMON_CHORDS.minors.find((item) => item.name === chordSymbol);
        if (!chordActive) {
            canClick = true;
            return;
        }
        ...
        if (isShowNote) {
            ...
            if (chordActive) {
                ...
                chordActive.strings.forEach((chord, index) => {
                    if (chord) {
                        $(
                            `.notes ${notesClassName[index]} ul li[note-number="${chord.fret}"]`
                        ).animate({ opacity: 1 }, 500);
                    }
                });
            } else {
